...
const AddEditExams = () => {
  ...
  useEffect(() => {
    if (examData) {
      formik.setValues({ name: examData.name, duration: examData.duration, category: examData.category, totalMarks: examData.totalMarks, passingMarks: examData.passingMarks })
    }
  }, [examData])
  ...
  return (
    <div className={styled.addExam_body}>
      <div className={styled.addExam_text}>
        <PageTitle title=
          {params.id ? "Imtahan düzəlişi" : "Imtahan əlavə et"}
        />
      </div>
      <div className={styled.form_body}>
        {(examData || !params.id) && <form onSubmit={formik.handleSubmit} >
            ...
            <TabPane tab='Imtahan Təfərrüatı' key='1'>
              <div className={styled.tab_pane1}>
                ...
                <div>
                  ...
                  <button className={styled.cancelBtn} type="button" onClick={()=> navigate('/admin/exams')}>Ləğv et</button>
                  <button className={styled.addBtn} type="submit">Əlavə et</button></div>
              </div>
